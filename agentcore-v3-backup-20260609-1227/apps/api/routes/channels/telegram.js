...
// POST /api/channels/telegram/mtproto-connect — подключение личного аккаунта (MTProto)
router.post('/mtproto-connect', authenticate, async (req, res) => {
  try {
    const { phoneNumber, code, password } = req.body;
    if (!phoneNumber) return res.status(400).json({ error: 'Номер телефона обязателен' });
    ...
    res.json({ 
      ok: true, 
      message: 'Данные MTProto сохранены. Внимание: использование личного аккаунта может привести к блокировке со стороны Telegram.' 
    });
  } catch (err) {
    console.error('[Telegram] Ошибка MTProto connect:', err.message);
    safeError(res, err, 500, 'Не удалось сохранить MTProto настройки.');
  }
});
