...
const { getAgentCredits } = require('../utils/credits');
...
router.post('/:id/messages', authenticate, aiLimiter, async (req, res) => {
  try {
    ...
    if (role === 'user') {
      ...
      // Check credits for the agent
      const credits = await getAgentCredits(conversation.agentId, req.user.workspaceId);
      if (credits.remaining < 5) {
        const errorMessage = await prisma.message.create({
          data: {
            content: 'Недостаточно AI-кредитов. Пополните баланс в настройках.',
            role: 'system',
            model: selectedModel,
