...
class AvitoProvider extends IntegrationProvider {
  ...
  /**
   * Валидация credentials перед сохранением — тестовый запрос к API.
   * @param {object} credentials
  ...
  async validateCredentials(credentials) {
    ...
    if (!credentials.userId) {
      return { valid: false, error: 'userId обязателен' };
    }
    if (!credentials.accessToken && !(credentials.clientId && credentials.clientSecret)) {
      return { valid: false, error: 'Необходимо указать accessToken либо пару clientId + clientSecret' };
    }
    if (credentials.accessToken) {
      try {
        const response = await axios.get(`${AVITO_API_BASE}/messenger/v2/accounts/${credentials.userId}/chats`, {
        ...
        return { valid: false, error: `Неверный accessToken: ${err.response?.data?.message || err.message}` };
      }
    }
    return { valid: true };
  }
