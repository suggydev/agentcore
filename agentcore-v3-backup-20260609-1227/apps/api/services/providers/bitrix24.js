...
class Bitrix24Provider extends IntegrationProvider {
   ...
   * Для webhook — проверка наличия данных.
   * Для OAuth — тестовый запрос user.current.
   * @param {object} credentials
  ...
  async validateCredentials(credentials) {
    ...
    if (!credentials.domain) {
      return { valid: false, error: 'domain обязателен' };
    }
    const domain = credentials.domain.replace(/\/+$/, '').replace(/^https?:\/\//, '');
    if (credentials.webhookUserId && credentials.webhookToken) {
      return { valid: true };
    }
    if (credentials.accessToken) {
      try {
        const response = await axios.post(
          `https://${domain}/rest/user.current`,
          { auth: credentials.accessToken },
          { headers: { 'Content-Type': 'application/json' }, timeout: 10000 }
        );
        if (response.data && response.data.error) {
          return { valid: false, error: `Неверный accessToken: ${response.data.error_description || response.data.error}` };
        }
  ...
  _extractEmail(emailField) {
    ...
    if (!message || typeof message !== 'string' || message.trim().length === 0) {
      throw new AppError('[Bitrix24] message is required', 400, 'VALIDATION_ERROR');
    }
    const dialogId = conversationId || 'admin';
    try {
      const result = await this._call('im.message.add', { DIALOG_ID: dialogId, MESSAGE: message.trim() });
      return { messageId: result, dialogId };
    } catch (err) {
      this.log('error', 'sendMessage failed', { error: err.message, dialogId });
      throw err;
    }
  }
