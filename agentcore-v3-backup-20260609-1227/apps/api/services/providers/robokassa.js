...
class RobokassaProvider extends IntegrationProvider {
  ...
  async initialize(credentials) {
    if (!credentials || typeof credentials !== 'object') {
      throw new AppError('[Robokassa] credentials должен быть объектом вида { merchantLogin, password1, password2 }', 400, 'MISSING_CONFIG');
    }
    if (!credentials.merchantLogin || typeof credentials.merchantLogin !== 'string') {
      throw new AppError('[Robokassa] Отсутствует обязательный параметр merchantLogin', 400, 'MISSING_CONFIG');
    }
    if (!credentials.password1 || typeof credentials.password1 !== 'string') {
      throw new AppError('[Robokassa] Отсутствует обязательный параметр password1', 400, 'MISSING_CONFIG');
    }
    if (!credentials.password2 || typeof credentials.password2 !== 'string') {
      throw new AppError('[Robokassa] Отсутствует обязательный параметр password2', 400, 'MISSING_CONFIG');
    }
    if (credentials.algorithm && !['md5', 'sha1', 'sha256'].includes(credentials.algorithm)) {
      throw new AppError('[Robokassa] Алгоритм подписи должен быть одним из: md5, sha1, sha256', 400, 'VALIDATION_ERROR');
    }
  ...
  async validateCredentials(credentials) {
    ...
    if (!credentials.merchantLogin || typeof credentials.merchantLogin !== 'string') {
      return { valid: false, error: 'merchantLogin обязателен' };
    }
    if (!credentials.password1 || typeof credentials.password1 !== 'string') {
      return { valid: false, error: 'password1 обязателен' };
    }
    if (!credentials.password2 || typeof credentials.password2 !== 'string') {
      return { valid: false, error: 'password2 обязателен' };
    }
    try {
      const response = await axios.get(`${ROBOKASSA_API_BASE}`, {
        params: {
          MerchantLogin: credentials.merchantLogin,
          Culture: 'ru',
          Encoding: 'utf-8',
          IsTest: credentials.isTest ? '1' : '0',
        },
        timeout: 10000,
      });
      ...
      return { valid: false, error: 'Неверные учётные данные Robokassa' };
    } catch (err) {
      return { valid: false, error: `Неверные учётные данные: ${err.response?.data?.result?.description || err.message}` };
    }
  }
  ...
  async createInvoice({
    ...
    if (!invoiceId || typeof invoiceId !== 'number') {
      throw new AppError('[Robokassa] Параметр invoiceId обязателен и должен быть числом', 400, 'VALIDATION_ERROR');
    }
    if (!outSum || typeof outSum !== 'number' || outSum <= 0) {
      throw new AppError('[Robokassa] Параметр outSum обязателен и должен быть положительным числом', 400, 'VALIDATION_ERROR');
    }
    if (!description || typeof description !== 'string') {
      throw new AppError('[Robokassa] Параметр description обязателен и должен быть строкой', 400, 'VALIDATION_ERROR');
    }
    ...
    if (receiptItems && Array.isArray(receiptItems) && receiptItems.length > 0) {
      const items = receiptItems.map(item => {
        if (!item.name || !item.quantity || !item.sum) {
          throw new AppError('[Robokassa] Каждый элемент чека должен содержать name, quantity и sum', 400, 'VALIDATION_ERROR');
        }
        return {
  ...
  async checkPaymentStatus({ invoiceId, outSum } = {}) {
    if (!invoiceId) {
      throw new AppError('[Robokassa] Параметр invoiceId обязателен для проверки статуса', 400, 'VALIDATION_ERROR');
    }
    if (outSum === undefined || outSum === null) {
      throw new AppError('[Robokassa] Параметр outSum обязателен для проверки статуса', 400, 'VALIDATION_ERROR');
    }
  ...
  async createRecurringPayment({
    ...
    if (!recurringAmount || typeof recurringAmount !== 'number' || recurringAmount <= 0) {
      throw new AppError('[Robokassa] Параметр recurringAmount обязателен и должен быть положительным числом', 400, 'VALIDATION_ERROR');
    }
    if (!nextPaymentDate || !/^\d{4}-\d{2}-\d{2}$/.test(nextPaymentDate)) {
      throw new AppError('[Robokassa] Параметр nextPaymentDate обязателен в формате YYYY-MM-DD', 400, 'VALIDATION_ERROR');
    }
    if (!description || typeof description !== 'string') {
  ...
  async cancelRecurringPayment(invoiceId) {
    if (!invoiceId) {
      throw new AppError('[Robokassa] Параметр invoiceId обязателен для отмены регулярного платежа', 400, 'VALIDATION_ERROR');
    }
    ...
    this.log('warn', 'sendMessage not implemented for Robokassa', { agentId, conversationId });
    return { success: true, message: 'Robokassa does not support messaging via API' };
  }

  async handleWebhook(payload, signature) {
    this.log('warn', 'handleWebhook not implemented for Robokassa');
    return { processed: true, event: payload?.event || 'unknown' };
  }
