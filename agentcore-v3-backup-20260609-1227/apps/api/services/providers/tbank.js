...
class TbankProvider extends IntegrationProvider {
  ...
  async initialize(credentials) {
    if (!credentials || typeof credentials !== 'object') {
      throw new AppError('[Т-Банк] credentials должен быть объектом вида { apiKey }', 400, 'MISSING_CONFIG');
    }
    if (!credentials.apiKey || typeof credentials.apiKey !== 'string' || credentials.apiKey.length < 10) {
      throw new AppError('[Т-Банк] Отсутствует или некорректен обязательный параметр apiKey', 400, 'MISSING_CONFIG');
    }
  ...
  /**
   * Валидация credentials перед сохранением — тестовый запрос к Т-Банк API.
   * @param {object} credentials
  ...
  async validateCredentials(credentials) {
    ...
    if (!credentials.apiKey || typeof credentials.apiKey !== 'string' || credentials.apiKey.length < 10) {
      return { valid: false, error: 'apiKey обязателен (минимум 10 символов)' };
    }
    try {
      const response = await axios.get(`${TBANK_API_BASE}/company`, {
        headers: { Authorization: `Bearer ${credentials.apiKey}`, Accept: 'application/json' },
        timeout: 10000,
      });
      if (response.data && (response.data.inn || response.data.name)) {
        return { valid: true };
      }
      return { valid: false, error: 'Неверный apiKey' };
    } catch (err) {
      return { valid: false, error: `Неверный apiKey: ${err.response?.data?.message || err.message}` };
    }
  }
  ...
  async getStatement({ accountNumber, dateFrom, dateTo } = {}) {
    ...
    this.log('warn', 'sendMessage not implemented for T-Bank', { agentId, conversationId });
    return { success: true, message: 'T-Bank does not support messaging via API' };
  }

  async handleWebhook(payload, signature) {
    this.log('warn', 'handleWebhook not implemented for T-Bank');
