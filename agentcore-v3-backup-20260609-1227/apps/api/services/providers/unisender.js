...
class UnisenderProvider extends IntegrationProvider {
  ...
  /**
   * Валидация credentials перед сохранением — тестовый запрос к API (getLists).
   * @param {object} credentials
  ...
  async validateCredentials(credentials) {
    ...
    try {
      const response = await axios.get(`${UNISENDER_API_BASE}/getLists`, {
        params: { format: 'json', api_key: credentials.apiKey },
        timeout: 10000,
      });
      ...
      return { valid: false, error: `Неверный apiKey: ${err.response?.data?.error?.message || err.message}` };
    }
  }
   ...
   */
    ...
    if (!message || typeof message !== 'string' || message.trim().length === 0) {
      throw new AppError('[Unisender] message is required', 400, 'VALIDATION_ERROR');
    }
