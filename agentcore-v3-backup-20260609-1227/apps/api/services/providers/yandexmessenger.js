...
class YandexMessengerProvider extends IntegrationProvider {
  constructor(config = {}) {
    ...
    this.mode = config.mode || 'org'; // 'org' | 'user'
  }
  ...
  async initialize(credentials) {
    try {
      ...
      this.mode = credentials.mode || this.mode || 'org';
      if (this.mode === 'org' && !credentials.orgId) {
        throw new Error('[YandexMessenger] orgId is required for org mode');
      }
      if (this.mode === 'org') {
      ...
      this.log('info', 'YandexMessenger initialized', { orgId: this.orgId, mode: this.mode });
      return true;
    } catch (err) {
  ...
  async validateCredentials(credentials) {
    ...
    if (credentials.accessToken) {
      try {
        const response = await axios.get(`${YANDEX_MESSENGER_API}/me`, {
  ...
  async sendMessage(agentId, conversationId, message) {
    try {
      ...
      let path;
      if (this.mode === 'org') {
        path = `/orgs/${this.orgId}/chats/${conversationId}/messages`;
      } else {
        path = `/users/${conversationId}/messages`;
      }

      const result = await this._request('POST', path, { text: message });
      this.log('info', 'Message sent', { conversationId, mode: this.mode });
      return { messageId: result.message_id || result.id, conversationId, mode: this.mode };
    } catch (err) {
  ...
  async healthCheck() {
    try {
      ...
      if (this.mode === 'org') {
        ...
        await this._request('GET', `/users/self/chats`, null);
      }
      return { ok: true, latency: Date.now() - start, mode: this.mode };
    } catch (err) {
      this.log('error', 'Health-check failed', { error: err.message });
      return { ok: false, error: err.message, mode: this.mode };
    }
  }

  /**
   * Получает информацию об организации в org-режиме.
   * @returns {Promise<{ok: boolean, result: object}>}
   */
  async getOrganizationInfo() {
    try {
      if (this.mode !== 'org') {
        throw new Error('[YandexMessenger] getOrganizationInfo доступен только в org-режиме');
      }
      if (!this.orgId) {
        throw new Error('[YandexMessenger] orgId не установлен');
      }
      const result = await this._request('GET', `/orgs/${this.orgId}`);
      this.log('info', 'Organization info retrieved', { orgId: this.orgId });
      return { ok: true, result };
    } catch (err) {
      this.log('error', 'getOrganizationInfo failed', { error: err.message });
      throw err;
    }
  }
