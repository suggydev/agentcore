...
function Sparkle({ angle, distance }: { angle: number; distance: number }) {
 ...
 return (
 <motion.div
 className="absolute rounded-full bg-[var(--brand)]/30"
...
export default function AgentBrainAnimation({ agentName, isVisible, onComplete }: AgentBrainAnimationProps) {
 ...
 >
 <div
 className="absolute inset-0 bg-[var(--accent-soft)] backdrop-blur-[4px]"
 style={{
 ...
 <motion.div
 key={node.id}
 className="absolute flex items-center justify-center rounded-lg bg-[var(--brand)] shadow-lg"
 style={{
 width: NODE_SIZE,
 height: NODE_SIZE,
 ...
 {phase >= 3 && (
 <motion.p
 className="absolute left-0 w-full text-center font-display text-lg font-bold tracking-display text-[var(--brand)]"
 ...
 >
 <motion.p
 className="font-display text-3xl font-bold tracking-display text-[var(--text)]"
