...
export default function AgentReadyScreen({ agent, testPrompts, onClose }: AgentReadyScreenProps) {
 ...
 'Мне нужна консультация',
  ...
  const fullBotReply = useMemo(() => generateBotReply(agent.name), [agent.name]);
 ...
 >
  <button className="flex h-8 w-8 items-center justify-center rounded-full bg-[var(--brand)]/25 text-white shadow-sm transition-transform hover:scale-105 active:scale-95 cursor-default" aria-label="Отправить сообщение">
