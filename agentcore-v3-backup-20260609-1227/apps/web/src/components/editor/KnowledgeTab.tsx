...
import { FileText, Link, HelpCircle, Plus, Search, Trash2, Upload, Globe, Pencil, Check, X } from 'lucide-react';
...
export default function KnowledgeTab({ agentId, token }: KnowledgeTabProps) {
  ...
  const handleStartEdit = useCallback((item: KnowledgeItem) => {
  ...
  const handleSaveEdit = useCallback(async () => {
    ...
    setEditSaving(true);
    try {
      const res = await fetch(`${API_BASE}/api/knowledge/${editingId}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
        body: JSON.stringify({ title: editQuestion.trim(), content: editAnswer.trim() }),
      });
      if (res.ok) {
        setItems((prev) =>
          prev.map((item) =>
            item.id === editingId
              ? { ...item, title: editQuestion.trim(), content: editAnswer.trim() }
              : item
          )
        );
        addToast({ variant: 'success', message: t('toast.success') });
    ...
    setEditSaving(false);
  }, [editingId, editQuestion, editAnswer, token, addToast, handleCancelEdit]);
  ...
  return (
    <div className="p-5 flex flex-col gap-5">
      ...
      {filtered.map((item) => {
        ...
        return (
          <Card key={item.id} className={`flex flex-col gap-2 ${isEditing ? 'ring-2 ring-[var(--brand)]' : ''}`}>
            {isEditing ? (
              <div className="flex flex-col gap-2">
                <input
                  type="text"
                ...
                <textarea
                  ...
                  rows={2}
                  className="w-full px-3 py-2 rounded-lg border border-[var(--border)] bg-[var(--surface)] text-[var(--text)] text-[13px] focus:outline-none focus:ring-2 focus:ring-[var(--brand)] resize-y"
                  placeholder="Ответ"
                ...
                <div className="flex items-center justify-end gap-2">
                  <button
                    ...
                    className="p-1.5 rounded-[var(--radius-sm)] text-[var(--text-muted)] hover:text-[var(--text)] hover:bg-[var(--surface-2)] transition-colors"
                    aria-label="Отмена"
                  >
                    <X size={14} />
                  </button>
                  <button
                    ...
                    disabled={editSaving}
                    className="p-1.5 rounded-[var(--radius-sm)] text-[var(--success)] hover:bg-[var(--surface-2)] transition-colors disabled:opacity-50"
              ...
              <div className="flex items-center gap-3">
                ...
                {item.type === 'qa' && (
                  <button
                    ...
                    className="p-1.5 rounded-[var(--radius-sm)] text-[var(--text-muted)] hover:text-[var(--brand)] hover:bg-[var(--surface-2)] transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--brand)]"
