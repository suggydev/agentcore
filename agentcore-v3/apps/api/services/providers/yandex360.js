...
class Yandex360Provider extends IntegrationProvider {
  ...
  async initialize(credentials) {
    if (!credentials || typeof credentials !== 'object') {
      throw new Error('[Яндекс 360] credentials должен быть объектом вида { clientId, clientSecret, orgId }');
    }
    if (!credentials.orgId || (typeof credentials.orgId !== 'string' && typeof credentials.orgId !== 'number')) {
      throw new Error('[Яндекс 360] Отсутствует обязательный параметр orgId — ID организации');
    }
    ...
    if (credentials.accessToken) {
      ...
      if (!credentials.clientId || !credentials.clientSecret) {
        throw new Error('[Яндекс 360] Необходимо указать clientId и clientSecret, либо готовый accessToken');
      }
  ...
  async _ensureToken() {
    ...
    if (!this.accessToken) {
      throw new Error('[Яндекс 360] Нет токена доступа. Вызовите initialize() или установите clientId/clientSecret');
    }
  }
  ...
  async _refreshAccessToken() {
    try {
      ...
      throw new Error(`[Яндекс 360] Ошибка обновления токена: ${err.response?.data?.error_description || err.message}`);
    }
  }
