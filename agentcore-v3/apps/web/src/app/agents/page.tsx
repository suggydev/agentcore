...
import { Brain, Plus, Loader2, ArrowLeft, Trash2, Edit3, Save, X } from 'lucide-react';
...
export default function AgentsPage() {
  ...
  const loadAgents = async () => {
    try {
      const res = await fetch('http://31.76.102.116:4000/api/agents', {
  ...
  const createAgent = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setNewAgent({ name: '', description: '', model: '', systemPrompt: 'You are a helpful AI assistant.', temperature: 0.7 });
      }
    } catch {}
    setCreating(false);
  };
  ...
  const deleteAgent = async (id: string) => {
    try {
      await fetch(`http://31.76.102.116:4000/api/agents/${id}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${token}` }
      });
      ...
      <div className="max-w-5xl mx-auto px-6 py-10">
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <h1 className="text-3xl font-bold text-mauve-900">AI Agents</h1>
          </div>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-4">
          {agents.map((agent, i) => (
            ...
            >
              ...
              <p className="text-mauve-500 text-sm mt-1">{agent.description || 'No description'}</p>
        ...
        {agents.length === 0 && (
          <div className="text-center py-16">
            ...
            <p className="text-mauve-500">No agents yet. Create your first AI agent above!</p>
          </div>
        )}
      </div>
    </div>
