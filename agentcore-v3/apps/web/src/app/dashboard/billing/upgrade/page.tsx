...
export default function UpgradePage() {
  ...
  const renderBool = (val: boolean | string) => {
    ...
    return <span className="text-sm font-medium text-ink-700">{val}</span>;
  };
    ...
    <>
      <div className="p-6 lg:p-10 max-w-7xl mx-auto">
        {redirectingPlan && (
          ...
          >
            <div className="bg-white rounded-2xl border border-mauve-100 shadow-xl p-10 text-center max-w-sm mx-auto">
              <Loader2 className="w-10 h-10 text-mauve-500 animate-spin mx-auto mb-4" />
              <h3 className="font-display font-semibold text-lg text-ink-900 mb-2">Перенаправление...</h3>
        ...
        <motion.div variants={container} initial="hidden" animate="show" className="mb-10">
          <motion.div variants={item}>
            <p className="text-[11px] font-semibold uppercase tracking-label text-mauve-500 mb-2">Обновление</p>
            <h1 className="font-display font-bold text-3xl text-ink-900 tracking-tight">Выберите план</h1>
            <p className="text-ink-500 mt-1 text-sm">Выберите план, подходящий для вашего бизнеса.</p>
          </motion.div>
        </motion.div>
        ...
        <motion.div variants={container} initial="hidden" animate="show" className="grid lg:grid-cols-3 gap-6 mb-12">
          {UPGRADE_PLANS.map((plan) => (
            ...
            >
              ...
              <h3 className="font-display font-semibold text-lg text-ink-900 mb-1">{plan.name}</h3>
              <div className="flex items-baseline gap-1 mb-3">
                <span className="font-mono font-bold text-3xl text-ink-900">{plan.price}</span>
                <span className="text-sm text-ink-500">{plan.period}</span>
              </div>
              <p className="text-sm text-ink-500 mb-5">{plan.description}</p>
              <button
              ...
              {plan.id === 'enterprise' && enterpriseOpen && (
                <motion.div
                  ...
                  className="mt-5 pt-5 border-t border-mauve-100 overflow-hidden"
                >
                  {enterpriseSubmitted ? (
                    <div className="text-center py-3">
                      ...
                      <p className="text-sm font-medium text-ink-700">Заявка отправлена!</p>
                      <p className="text-xs text-ink-400 mt-1">Мы свяжемся с вами в ближайшее время.</p>
                    </div>
                  ) : (
                    <div className="space-y-3">
                      <input
                        ...
                        className="w-full px-3 py-2 rounded-lg border border-mauve-200 text-sm text-ink-700 placeholder:text-ink-400 focus:outline-none focus:border-mauve-400 focus:ring-1 focus:ring-mauve-400/20 transition-colors"
                      />
                      <input
                        type="email"
                        placeholder="Email"
                      ...
                      <textarea
                        ...
                        className="w-full px-3 py-2 rounded-lg border border-mauve-200 text-sm text-ink-700 placeholder:text-ink-400 focus:outline-none focus:border-mauve-400 focus:ring-1 focus:ring-mauve-400/20 transition-colors resize-none"
                      />
                      <button
                        onClick={handleEnterpriseSubmit}
                        className="w-full py-2.5 rounded-xl bg-ink-900 text-white text-sm font-semibold hover:bg-ink-800 transition-colors duration-200 flex items-center justify-center gap-2"
                      >
                      ...
                      <p className="text-[10px] text-ink-400 text-center">
                        ...
                        <a
                          ...
                          className="text-mauve-600 hover:text-mauve-700 underline"
                        >
                          {ENTERPRISE_CONTACT_EMAIL}
                        </a>
                      </p>
              ...
              <ul className="space-y-2.5 mt-6">
                {plan.features.map((f, i) => (
                  <li key={i} className="flex items-center gap-2 text-sm text-ink-600">
                    <Check className={`w-4 h-4 ${plan.popular ? 'text-mauve-500' : 'text-mauve-400'} flex-shrink-0`} />
                    {f}
                  </li>
                ))}
              </ul>
        ...
        <motion.div variants={container} initial="hidden" animate="show" className="bg-white rounded-2xl border border-mauve-100 shadow-sm p-6 mb-8">
          <h2 className="font-display font-semibold text-lg text-ink-900 mb-5">Сравнение функций</h2>
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead>
                <tr className="border-b border-ink-100">
                  <th className="text-left py-3 text-xs font-semibold text-ink-500 uppercase tracking-wide">Функция</th>
                  <th className="text-center py-3 text-xs font-semibold text-ink-500 uppercase tracking-wide w-24">Starter</th>
                ...
                {comparisonRows.map((row, i) => (
                  ...
                  >
                    <td className="py-3 flex items-center gap-2">
                      ...
                      <span className="text-sm font-medium text-ink-700">{row.feature}</span>
                    </td>
                    <td className="py-3 text-center">{renderBool(row.starter)}</td>
                    <td className="py-3 text-center bg-mauve-50/30">{renderBool(row.pro)}</td>
        ...
        <motion.div variants={item} initial="hidden" animate="show" className="text-center pb-8">
          <div className="inline-flex items-center gap-2 px-5 py-3 rounded-2xl bg-mauve-50 border border-mauve-100">
            <Shield className="w-5 h-5 text-mauve-600" />
            <span className="text-sm text-mauve-700 font-medium">
