...
export default function KnowledgePage() {
 ...
 className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-[var(--accent)] text-white text-sm font-semibold hover:bg-[var(--accent)] transition-all duration-200 shadow-sm"
 >
 ...
 <p className="text-xs text-[var(--text-muted)] flex items-center gap-1">{stat.label} <InfoTooltip content={stat.tooltip} iconSize={12} /></p>
 </div>
 </motion.div>
 ))}
 </motion.div>
 ...
 className="absolute inset-0 flex items-center justify-center rounded-2xl bg-[var(--brand)]/5 backdrop-blur-[1px]"
 >
 <div className="flex flex-col items-center gap-3">
 <div className="w-16 h-16 rounded-2xl bg-[var(--brand)]/10 flex items-center justify-center ring-2 ring-[var(--brand)]/20 animate-pulse">
 <Upload className="w-7 h-7 text-[var(--brand)]" />
 </div>
 <p className="text-[var(--brand)] font-bold text-base">Отпустите, чтобы загрузить</p>
 </div>
 </motion.div>
 )}
 ...
 <h2 className="font-display font-semibold text-lg text-[var(--text)]">Документы</h2>
 ...
 className="absolute top-0 left-0 right-0 h-0.5 bg-gradient-to-r from-[var(--brand)]/0 via-[var(--brand)]/40 to-[var(--brand)]/0 origin-left"
 />
 ...
 <span className="text-xs text-[var(--text-muted)] italic">Заполните вопрос и ответ</span>
 )}
 </div>
