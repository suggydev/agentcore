...
export default function LoginPage() {
  ...
  const renderLogin = () => (
    <form onSubmit={handleLogin} className="space-y-4">
      ...
      <div>
        <label className="block text-sm font-medium text-ink-700 mb-1.5">Пароль</label>
  ...
  const renderStep1 = () => (
    <motion.div key="step1" {...slideRight} className="space-y-4">
      <div>
        <label className="block text-sm font-medium text-ink-700 mb-1.5">Как вас зовут?</label>
        <input type="text" value={name} onChange={e => setName(e.target.value)}
          className={inputClass} placeholder="Иван Иванов" autoFocus />
      </div>
      <div>
        <label className="block text-sm font-medium text-ink-700 mb-1.5">Рабочий email</label>
      ...
      <div>
        ...
        {getPasswordStrength(password).score > 0 && (
          <div className="flex items-center gap-1.5 mt-1.5">
            ...
            <span className="text-[10px] text-ink-400">{getPasswordStrength(password).label}</span>
          </div>
        )}
      </div>
      {stepError && (
  ...
  const renderStep2 = () => (
    <motion.div key="step2" {...slideRight} className="space-y-4">
      <div>
        <label className="block text-sm font-medium text-ink-700 mb-1.5">Название компании</label>
        <div className="relative">
          <Building2 className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-ink-300" />
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="block text-sm font-medium text-ink-700 mb-1.5">Размер команды</label>
          <div className="relative">
            <Users className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-ink-300" />
        ...
        <div>
          <label className="block text-sm font-medium text-ink-700 mb-1.5">Индустрия</label>
          <div className="relative">
            <Briefcase className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-ink-300" />
      ...
      <div>
        <label className="block text-sm font-medium text-ink-700 mb-1.5">Откуда вы о нас узнали?</label>
        <div className="flex flex-wrap gap-2">
          {SOURCES.map(({ id, icon: Icon, label }) => (
            <button key={id} type="button" onClick={() => setSource(id)}
              className={`flex items-center gap-1.5 px-3 py-2 rounded-xl text-sm font-medium transition-all border
                ${source === id
                  ? 'bg-mauve-500 text-white border-mauve-500 shadow-sm'
                  : 'bg-white text-ink-600 border-mauve-200 hover:border-mauve-300 hover:bg-mauve-50/50'}`}>
      ...
      <div>
        <label className="block text-sm font-medium text-ink-700 mb-1.5">Для чего вам AI-агент?</label>
          ...
          {PURPOSES.map(({ id, icon: Icon, label, desc }) => (
            <button key={id} type="button" onClick={() => setPurpose(id)}
              className={`flex items-start gap-3 p-3 rounded-xl text-left transition-all border
                ${purpose === id
                  ? 'bg-mauve-50 border-mauve-400 ring-1 ring-mauve-400/20'
                  : 'bg-white border-mauve-200 hover:border-mauve-300 hover:bg-mauve-50/30'}`}>
              <Icon className={`w-5 h-5 mt-0.5 flex-shrink-0 ${purpose === id ? 'text-mauve-600' : 'text-mauve-400'}`} />
              <div>
      ...
      <div className="flex gap-3 pt-1">
        <button onClick={prevStep}
          className="flex items-center gap-1.5 px-5 py-3 rounded-xl border border-mauve-200 text-ink-600 hover:bg-mauve-50 transition-all text-sm font-medium">
    ...
    <motion.div key="step3" {...slideRight} className="space-y-5">
      <div className="bg-white rounded-2xl p-5 border border-mauve-100 shadow-sm">
        <h3 className="text-sm font-semibold text-ink-900 mb-4 flex items-center gap-2">
          <div className="w-6 h-6 rounded-full bg-mauve-100 flex items-center justify-center">
            <Check className="w-3.5 h-3.5 text-mauve-600" />
          </div>
          Проверьте данные
        </h3>
        <div className="space-y-2.5 text-sm">
          <div className="flex justify-between items-center py-1.5 px-3 rounded-lg bg-mauve-50/40">
            <span className="text-ink-500">Имя</span>
          ...
          {companyName && (
            <div className="flex justify-between items-center py-1.5 px-3 rounded-lg bg-mauve-50/40">
              <span className="text-ink-500">Компания</span>
              <span className="font-medium text-ink-900">{companyName}</span>
            </div>
          )}
          {companySize && (
            <div className="flex justify-between items-center py-1.5 px-3 rounded-lg bg-mauve-50/40">
              <span className="text-ink-500">Команда</span>
              <span className="font-medium text-ink-900">{companySize} чел.</span>
            </div>
          )}
          {industry && (
            <div className="flex justify-between items-center py-1.5 px-3 rounded-lg bg-mauve-50/40">
              <span className="text-ink-500">Индустрия</span>
              <span className="font-medium text-ink-900">{industry}</span>
            </div>
          )}
          {purpose && (
            <div className="flex justify-between items-center py-1.5 px-3 rounded-lg bg-mauve-50/40">
              <span className="text-ink-500">Цель</span>
              <span className="font-medium text-ink-900">{PURPOSES.find(p => p.id === purpose)?.label}</span>
            </div>
          )}
        </div>
      </div>
      ...
      <p className="text-center text-xs text-ink-400 leading-relaxed">
        ...
        <a href="/privacy" className="text-mauve-500 hover:text-mauve-700 underline underline-offset-2">Политикой конфиденциальности</a>
        {' '}и{' '}
        <a href="/terms" className="text-mauve-500 hover:text-mauve-700 underline underline-offset-2">Условиями использования</a>
      </p>
    </motion.div>
  );
    ...
    <div className="flex items-center justify-center mb-6">
      {STEPS.map((s, i) => (
        <div key={s.num} className="flex items-center">
          ...
          <span className={`text-xs font-medium hidden sm:block ml-2 mr-1 ${i <= step ? 'text-ink-700' : 'text-ink-300'}`}>
            {s.label}
          </span>
          {i < STEPS.length - 1 && (
            <div className="relative w-8 sm:w-12 h-0.5 rounded-full bg-mauve-200 overflow-hidden mx-1">
              <motion.div
                initial={false}
                animate={{ width: i < step ? '100%' : '0%' }}
                transition={{ duration: 0.4, ease: [0.16, 1, 0.3, 1] }}
                className="absolute inset-0 bg-mauve-600 rounded-full"
              />
            </div>
          )}
        </div>
  ...
  const renderLoginHeader = () => (
    <div className="text-center mb-8">
      <div className="inline-flex items-center gap-2 mb-1">
        <div className="w-2 h-2 rounded-full bg-mauve-500 animate-pulse" />
        <span className="text-xs font-semibold text-mauve-500 uppercase tracking-wider">AgentCore</span>
      </div>
      <h1 className="font-display font-bold text-2xl text-ink-900 mb-2">
        С возвращением
      </h1>
      <p className="text-ink-400 text-sm">Войдите в рабочую область</p>
    </div>
  );
  ...
  const renderRegisterHeader = () => (
    <div className="text-center mb-2">
      <div className="inline-flex items-center gap-2 mb-1">
        <Heart className="w-4 h-4 text-mauve-500" />
        <span className="text-xs font-semibold text-mauve-500 uppercase tracking-wider">AgentCore</span>
      </div>
