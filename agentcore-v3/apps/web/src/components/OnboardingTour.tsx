  ...
  return (
    <AnimatePresence>
      <div className="fixed inset-0 z-[9999] flex items-center justify-center" style={{ pointerEvents: 'all' }}>
        ...
        >
          <div className="bg-white rounded-2xl shadow-2xl border border-mauve-200/60 overflow-hidden">
            {/* Header */}
            <div className="flex items-center justify-between px-5 pt-5 pb-2">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-mauve-500 to-mauve-600 flex items-center justify-center">
                  <Icon className="w-5 h-5 text-white" />
                </div>
                <span className="text-xs font-semibold uppercase tracking-wider text-mauve-500">
                  Шаг {step + 1} из {STEPS.length}
                </span>
              </div>
              <button
                onClick={skip}
                className="p-1.5 rounded-lg text-ink-300 hover:text-ink-600 hover:bg-ink-50 transition-colors"
            ...
            {/* Footer */}
            <div className="flex items-center justify-between px-5 py-4 border-t border-mauve-100/60">
              ...
              <div className="flex items-center gap-2">
                <button
                  onClick={skip}
                  className="px-3 py-2 rounded-xl text-sm font-medium text-ink-400 hover:text-ink-600 hover:bg-ink-50 transition-colors"
                >
                  Пропустить
                </button>
                <button
                  ...
                  className={`flex items-center gap-1.5 px-5 py-2.5 rounded-xl text-sm font-semibold transition-all ${
                    isLast
                      ? 'bg-gradient-to-r from-mauve-600 to-mauve-700 text-white hover:shadow-lg hover:shadow-mauve-600/20'
