...
export function getJwtSecret(): string {
  ...
  if (config.AGENTCORE_ENCRYPTION_KEY) return config.AGENTCORE_ENCRYPTION_KEY;
  if (config.SUPABASE_SERVICE_ROLE_KEY) return config.SUPABASE_SERVICE_ROLE_KEY;
  throw new Error("JWT_SECRET or AGENTCORE_ENCRYPTION_KEY is required for authentication");
}
